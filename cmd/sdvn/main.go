...
// sdvn is the official command-line client for sdvn.
package main
...
import (
	...
	"github.com/seaskycheng/sdvn/eth"
	...
	"github.com/seaskycheng/sdvn/internal/ethapi"
	"github.com/seaskycheng/sdvn/internal/flags"
...
const (
	clientIdentifier = "sdvn" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the sdvn command line interface")
...
func init() {
	// Initialize the CLI app and start sdvn
	app.Action = sdvn
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2021 The sdvn Authors"
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting sdvn on sdvn mainnet...")
	}
...
// sdvn is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func sdvn(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	// Create a client to interact with local sdvn node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		if !ok {
			utils.Fatalf("sdvn service not running: %v", err)
		}
