...
func encodeHeaderExtra(config *params.AlienConfig, number *big.Int, val HeaderExtra) ([]byte, error) {
	...
	default:
		if isLtFulTrieNumber(number.Uint64()){
			oldHeaderExtra:=copyToOldHeaderExtra(val)
...
func copyToOldHeaderExtra(val2 HeaderExtra) OldHeaderExtra{
	val:=OldHeaderExtra{}
	val.CurrentBlockConfirmations=val2.CurrentBlockConfirmations
	val.CurrentBlockVotes=val2.CurrentBlockVotes
	val.CurrentBlockProposals= val2.CurrentBlockProposals
	val.CurrentBlockDeclares=val2.CurrentBlockDeclares
	val.ModifyPredecessorVotes=val2.ModifyPredecessorVotes
	val.LoopStartTime=  val2.LoopStartTime
	val.SignerQueue= val2.SignerQueue
	val.SignerMissing=val2.SignerMissing
	val.ConfirmedBlockNumber=val2.ConfirmedBlockNumber
	val.SideChainConfirmations=val2.SideChainConfirmations
	val.SideChainSetCoinbases=val2.SideChainSetCoinbases
	val.SideChainNoticeConfirmed=val2.SideChainNoticeConfirmed
	val.SideChainCharging= val2.SideChainCharging
	val.ExchangeNFC=val2.ExchangeNFC
	val.DeviceBind=val2.DeviceBind
	val.CandidatePledge=val2.CandidatePledge
	val.CandidatePunish=val2.CandidatePunish
	val.MinerStake=val2.MinerStake
	val.CandidateExit=val2.CandidateExit
	val.ClaimedBandwidth= val2.ClaimedBandwidth
	val.FlowMinerExit= val2.FlowMinerExit
	val.BandwidthPunish= val2.BandwidthPunish
	val.ConfigExchRate= val2.ConfigExchRate
	val.ConfigOffLine=val2.ConfigOffLine
	val.ConfigDeposit=val2.ConfigDeposit
	val.ConfigISPQOS=val2.ConfigISPQOS
	val.LockParameters=val2.LockParameters
	val.ManagerAddress= val2.ManagerAddress
	val.FlowHarvest=val2.FlowHarvest
	val.LockReward=val2.LockReward
	val.GrantProfit=val2.GrantProfit
	val.FlowReport=val2.FlowReport
	return val
}
...
func decodeHeaderExtra(config *params.AlienConfig, number *big.Int, b []byte, val *HeaderExtra) error {
	...
	default:
		if isLtFulTrieNumber(number.Uint64()){
			oldVal := &OldHeaderExtra{}
			err = rlp.DecodeBytes(b, oldVal)
			copyFromOldHeaderExtra(val,oldVal)
		}else{
...
func copyFromOldHeaderExtra(val *HeaderExtra, oldVal *OldHeaderExtra) {
	val.CurrentBlockConfirmations=oldVal.CurrentBlockConfirmations
	val.CurrentBlockVotes=oldVal.CurrentBlockVotes
	val.CurrentBlockProposals= oldVal.CurrentBlockProposals
	val.CurrentBlockDeclares=oldVal.CurrentBlockDeclares
	val.ModifyPredecessorVotes=oldVal.ModifyPredecessorVotes
	val.LoopStartTime=  oldVal.LoopStartTime
	val.SignerQueue= oldVal.SignerQueue
	val.SignerMissing=oldVal.SignerMissing
	val.ConfirmedBlockNumber=oldVal.ConfirmedBlockNumber
	val.SideChainConfirmations=oldVal.SideChainConfirmations
	val.SideChainSetCoinbases=oldVal.SideChainSetCoinbases
	val.SideChainNoticeConfirmed=oldVal.SideChainNoticeConfirmed
	val.SideChainCharging= oldVal.SideChainCharging
	val.ExchangeNFC=oldVal.ExchangeNFC
	val.DeviceBind=oldVal.DeviceBind
	val.CandidatePledge=oldVal.CandidatePledge
	val.CandidatePunish=oldVal.CandidatePunish
	val.MinerStake=oldVal.MinerStake
	val.CandidateExit=oldVal.CandidateExit
	val.ClaimedBandwidth= oldVal.ClaimedBandwidth
	val.FlowMinerExit= oldVal.FlowMinerExit
	val.BandwidthPunish= oldVal.BandwidthPunish
	val.ConfigExchRate= oldVal.ConfigExchRate
	val.ConfigOffLine=oldVal.ConfigOffLine
	val.ConfigDeposit=oldVal.ConfigDeposit
	val.ConfigISPQOS=oldVal.ConfigISPQOS
	val.LockParameters=oldVal.LockParameters
	val.ManagerAddress= oldVal.ManagerAddress
	val.FlowHarvest=oldVal.FlowHarvest
	val.LockReward=oldVal.LockReward
	val.GrantProfit=oldVal.GrantProfit
	val.FlowReport=oldVal.FlowReport
}
...
func (a *Alien) processCustomTx(headerExtra HeaderExtra, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, receipts []*types.Receipt) (HeaderExtra, RefundGas, error) {
	...
	fulBalances:= make(map[common.Address]*big.Int)
	for _, tx := range txs {
				...
				if txDataInfo[posPrefix] == ufoPrefix {
					if txDataInfo[posVersion] == ufoVersion {
						...
						if txDataInfo[posCategory] == ufoCategoryEvent {
							...
							if len(txDataInfo) > ufoMinSplitLen {
								if txDataInfo[posEventConfirm] == ufoEventConfirm {
								...
								} else if txDataInfo[posEventSetCoinbase] == ufoEventSetCoinbase && a.isManagerAddressFlowReport(txSender,snap) {
								...
								} else if txDataInfo[posEventSetCoinbase] == ufoEventDelCoinbase && a.isManagerAddressFlowReport(txSender,snap) {
									if len(txDataInfo) > ufoMinSplitLen+1 {
										headerExtra.SideChainSetCoinbases = a.processSCEventSetCoinbase(headerExtra.SideChainSetCoinbases,
									...
									ok := false
									headerExtra.FlowReport, ok = a.processFlowReport1 (headerExtra.FlowReport, txDataInfo, txSender, snap,number)
									if ok {
										refundHash[tx.Hash()] = RefundPair{txSender, tx.GasPrice()}
									}
								} else if ufoEventFlowReport2 == txDataInfo[posEventFlowReport] && a.isManagerAddressFlowReport(txSender,snap) {
										headerExtra.FlowReport = a.processFlowReport2 (headerExtra.FlowReport, txDataInfo,number,snap)
										refundHash[tx.Hash()] = RefundPair{txSender, tx.GasPrice()}
								}
							}
						}
					...
					if txDataInfo[posVersion] == ufoVersion {
						...
						if isGeFulTrieNumber(number){
							headerExtra=a.processFlowCustomTx(txDataInfo,headerExtra,txSender, tx, receipts, snapCache, header.Number,state,chain,fulBalances)
						}
					}
...
func (a *Alien) processMinerPledge (currentClaimedBandwidth []ClaimedBandwidthRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []ClaimedBandwidthRecord {
	...
	if 0 < snap.FlowTotal.Cmp(big.NewInt(1073741824 * 1024)) {
		...
		claimedBandwidth.Amount =new(big.Int).Div( new(big.Int).Mul(big.NewInt(int64(bandwidth)),new(big.Int).Mul(big.NewInt(int64(unitPrice)),big.NewInt(1e+18))),big.NewInt(100))
	}
...
func (a *Alien) processFlowReport1(flowReport []MinerFlowReportRecord, txDataInfo []string, txSender common.Address, snap *Snapshot, number uint64) ([]MinerFlowReportRecord, bool) {
	...
	if err := rlp.DecodeBytes(common.FromHex(txDataInfo[posEventFlowValue]), &report); err == nil {
		if isGeFulTrieNumber(number){
            if snap.CheckFulEnough(flowReport,report) {
				flowReport = append(flowReport, report)
				ok = true
			}else{
				log.Warn("processFlowReport1 ", "err", "CheckFulEnough fail")
			}
		}else{
...
func (a *Alien) processFlowReport2(flowReport []MinerFlowReportRecord, txDataInfo []string, number uint64, snap *Snapshot) []MinerFlowReportRecord {
	if len(txDataInfo) <= posEventFlowValue {
		log.Warn("Flow report", "parameter number", len(txDataInfo))
		return flowReport
	}
	...
	if isGeFulTrieNumber(number){
		if snap.CheckFulEnough(flowReport,census) {
			flowReport = append(flowReport, census)
		}else{
			log.Warn("processFlowReport2 ", "err", "CheckFulEnough fail")
		}
	}else{
		flowReport = append(flowReport, census)
	}
