...
var (
	...
	managerAddressExchRate           = common.HexToAddress("0x054ebaa7d5cdf0fb2efe237deb68b888fbcf6e11") ////TODO seaskycheng
	managerAddressSystem             = common.HexToAddress("0x01ebc48ef1a33c401c786494581473925bec5d67") ////TODO seaskycheng
	managerAddressWdthPnsh           = common.HexToAddress("0x0f0635247686493bbb2498103e24cf7dc548ac2a") ////TODO seaskycheng
	managerAddressFlowReport         = common.HexToAddress("0xce802718f278b16d679dec823cde21b945affdc7") ////TODO seaskycheng
	managerAddressManager            = common.HexToAddress("0xa34acfa918a7a82256027bd4d8e41a4c804c2688") ////TODO seaskycheng
)
...
func (a *Alien) VerifyLockReward(header *types.Header, snap *Snapshot, chain consensus.ChainHeaderReader, currentHeaderExtra HeaderExtra) error {
	...
	if a.isAccumulateFlowRewards(header.Number.Uint64()) {
		...
		lockRewards = accumulateBandwidthRewards(lockRewards, chain.Config(), header, snap, a.db)
	}
...
func (a *Alien) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, grantProfit []consensus.GrantProfitRecord, gasReward *big.Int) error {
	...
	if !chain.Config().Alien.SideChain {
		...
		if number == 1 {
			...
			//snap1.updateSignerNumber(currentHeaderExtra.SignerQueue)
		...
		if a.isAccumulateFlowRewards(header.Number.Uint64()) {
			...
			currentHeaderExtra.LockReward = accumulateBandwidthRewards(currentHeaderExtra.LockReward, chain.Config(), header, snap, a.db)
		}
		...
		if isGeFulTrieNumber(number){
			snap1 := snap.copy()
			currentHeaderExtra.FulDataRoot = snap1.Ful.Root()
		}
	} else {
...
func nYearBandwidthReward(n float64) decimal.Decimal {
	onecut := float64(1) - math.Pow(float64(0.5), n/float64(12))
	yearScale := decimal.NewFromFloat(onecut)
	yearReward := yearScale.Mul(decimal.NewFromBigInt(totalBandwidthReward, 0))
	return yearReward
}

func accumulateBandwidthRewards(currentLockReward []LockRewardRecord, config *params.ChainConfig, header *types.Header, snap *Snapshot, db ethdb.Database) []LockRewardRecord {
	...
	for minerAddress, bandwidth := range snap.Bandwidth {
		//Only miners who upload traffic can have bandwidth rewards
		if _, ok := flowcensus[minerAddress]; !ok || flowcensus[minerAddress].FlowValue1 <= 0 {
			continue
		}
		reward := big.NewInt(0)
		if bandwidth.BandwidthClaimed > 1500 {
...
func accumulateFlowRewards(currentLockReward []LockRewardRecord, snap *Snapshot, db ethdb.Database) ([]LockRewardRecord, *big.Int) {
	...
	for minerAddress, bandwidth := range flowcensus {
		if claimed, ok := snap.Bandwidth[minerAddress]; ok {
			...
			validFlow:=getBandwidthValidFlow(claimed,bandwidth)
...
func getBandwidthValidFlow(claimed *ClaimedBandwidth, bandwidth *FlowMinerReport) uint64 {
